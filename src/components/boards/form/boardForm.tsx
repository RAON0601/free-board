...
import { ButtonBlack, CreateBoardButton } from '@/components/commons/button.style';
    ...
    <Form onSubmit={onSubmit}>
      ...
      <Row>
        <InputField>
          ...
          />
          {errors.contents && <InputError>{`${errors.contents.message}`}</InputError>}
        </InputField>
      </Row>
      ...
      <Row style={{ justifyContent: 'start', alignItems: 'center' }}>
        <InputFieldSmall style={{ marginRight: '16px' }}>
          ...
          {errors.address1 && <InputError>{`${errors.address1.message}`}</InputError>}
        </InputFieldSmall>
      ...
      <Row>
        <InputField>
          ...
          {errors.address2 && <InputError>{`${errors.address2.message}`}</InputError>}
        </InputField>
      </Row>
      ...
      <Row>
        <InputField>
          ...
          {errors.address3 && <InputError>{`${errors.address3.message}`}</InputError>}
        </InputField>
      </Row>
      ...
      <Row>
        <InputField>
          <Label>유튜브</Label>
          <Input placeholder="링크를 복사해주세요" {...register('youtubeLink')} />
          {errors.youtubeLink && <InputError>{`${errors.youtubeLink.message}`}</InputError>}
        </InputField>
      </Row>
      ...
      <Row style={{ justifyContent: 'center', marginTop: '48px' }}>
        <CreateBoardButton type="submit" backgroundColor={validateInput() ? '#ffd600' : 'gray'}>
          {isEdit ? '수정' : '등록'}하기
        </CreateBoardButton>
      </Row>
    </Form>
