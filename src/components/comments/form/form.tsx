...
import { CommentFormProps } from "../write/write.types";
import {
  ButtonWrapper,
  Form,
  SubmitButton,
    ...
    <Form onSubmit={onSubmit}>
      ...
      <WriterInputWrapper>
        <InputFieldSmall>
          <Input
            placeholder="작성자"
            {...register("writer", { required: "작성자는 필수 입력값입니다!" })}
          />
      ...
      {errors.contents?.type === "required" && (
        <InputError>댓글을 작성해주세요!</InputError>
      )}
      {errors.contents?.type === "maxLength" && (
        <InputError>최대 100자만 입력 가능합니다</InputError>
      )}
    </Form>
  );
}
